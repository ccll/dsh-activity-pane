...
export default async function errorReminder({ page, url, mock, assert }) {
	...
	const errorElapsed = await until("错误提醒保留上一轮耗时", () =>
		pane.locator(`[role="button"]`).filter({ hasText: TITLE }).first().locator(".dap-await-head .dap-token-time").textContent().then((text) => (text ?? "").trim() || null),
	);
	...
	const errorStats = await until("错误提醒统计行状态就绪", () =>
		pane.locator(`[role="button"]`).filter({ hasText: TITLE }).first().locator(".dap-token-stats").evaluate((stats) => ({
			hidden: stats.hidden,
