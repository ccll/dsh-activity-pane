...
| R-01-012 | 活动状态模型 | 原生会话工作项、指令锚行、模型上下文（含子代理模型溯源与 reasoning effort）与实时快照 | src/core.mjs、src/client.mjs |
