...
- 后台任务输出通道契约（宿主侧，实现 R-01-024）：`GET /dsh-activity-pane/api/jobs-output` 接收 `{ sessionId, jobId }`，返回 `{ text, truncated, read }`——归属会话已完成的 `job_output` 读取回放（归属会话事件日志与内存镜像按 seq 去重合并、callId 配对拼接，剔除 error 结果与 `(no new output)` 占位；超出上限按字符截断并置 `truncated`；`read` = 存在已配对读取，无配对读取时 `text` 为空、由客户端以「尚未被读取」提示承接）；`GET /dsh-activity-pane/api/jobs/stream` 为 SSE 通知（连接即发空快照；新轨迹到达即广播 `{ sessionId, jobId }`）；客户端激活任务行时读取一次，此后仅随 jobs/stream 事件刷新，不引入状态轮询（R-01-024/AC-01～AC-03）。数据层配对逻辑收敛于 core 纯函数 `jobOutputTraces(events)`/`jobOutputFromTraces(traces, jobId, limit)`，宿主路由与单测共用同一实现。（宿主侧路由与协议，属完成确认宿主侧子系统。）
...
### 完成确认宿主侧
- 职责: 把会话回合结束、结束原因与用户确认持久化为 ack 状态并经 HTTP/SSE 通道下发（实现 R-01-002/AC-03、AC-05、AC-10～AC-13、R-01-010/AC-06）；镜像会话事件中的 `job_output` 读写轨迹并经只读路由回放后台任务输出（共同实现 R-01-024）
- 关键内部结构:
  ...
  - 后台任务输出镜像（实现 R-01-024）：同一 `session/event` 订阅中，`tool/call` 名为 `job_output` 的行经 `arguments.job_id` 登记 callId → jobId 映射，配对的 `tool/result` 行按 callId 归属提取 tool-result 文本块（剔除 error 结果与 `(no new output)` 占位）；每会话内存环形缓冲（200 条）承载，不持久化、不消费模型的 `job_output` 读取游标、零 DSH 写入；`GET /dsh-activity-pane/api/jobs-output` 以归属会话的事件日志（`sessionQuery.listEvents` 重放，宿主重启后的持久种子）与内存镜像按 seq 去重合并，配对产出 `{ text, truncated, read }`（`read` = 是否存在已配对的 tool/result，文本为空时客户端以「尚未被读取」提示承接）；新轨迹到达时经 `GET /dsh-activity-pane/api/jobs/stream` SSE 广播 `{ sessionId, jobId }` 事件（连接即发空快照，无全量快照语义——轨迹不入库，仅需通知客户端回读）。
  ...
  - 内存态仅连接集合与任务输出镜像环形缓冲；确认状态本身持久化于 storageDomain，宿主重启不丢；输出镜像重启后由会话事件日志重放补齐。
...
### 活动状态模型
- 职责: 把宿主会话与工作区快照归一化为活动区/历史区条目、工作项时间线、折叠分组时间线、模型上下文与消息预览（实现 R-01-001、R-01-002、R-01-003、R-01-009、R-01-010、R-01-012、R-01-013、R-01-016、R-01-017、R-01-020、R-01-023、R-01-024、R-02-001、R-02-003）
- 关键内部结构:
  - 纯函数、无 DOM、可单测。
