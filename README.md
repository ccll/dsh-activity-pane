...
- **运行卡对齐 `dsh-answer-pet` 的卡片**：状态行（进行中的工具名 / 正在回复 / 运行中 · 输出 token 计数 · 输出速率 · 已运行时长）+ 工具白名单参数摘要（path/url/query 等，不含完整命令或原始 JSON）+ 阶段进度条（同回合不倒退、工具阶段冻结、回合切换重置）+ 最近流程节点轨迹（已定案工具调用与当前阶段，含状态与耗时）。
