...
- [x] **展示当前工作与运行统计**：活动卡以最多 4 行折叠时间线展示最近指令、思考与工具调用；运行中卡片还显示回合进度、输出速率、缓存命中率、输入/输出 token 与运行时长，进入完成、阻塞或错误等待后保留上一轮耗时及最后已知的 tok/s、缓存、输入/输出统计；迁入最近历史后，历史卡在助手预览与活动时间之间继续保留最后一轮的可用统计。
