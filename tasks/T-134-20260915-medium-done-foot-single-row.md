...
状态: active
关联: R-01-021/AC-08 → 窗格渲染器；R-01-002/AC-09（措辞限定）
风险等级: standard
...
- PRD：R-01-021/AC-08 补中间档完成提醒收合句；R-01-002/AC-09 限定完整呈现档。
...
| R-01-002/AC-09 | 措辞澄清：完成提醒末行两行结构限定完整呈现档（完整档行为不变） | manual | none | 完整档呈现与按钮行为不变，既有验收证据（`scripts/acceptance.mjs#R-01-002/AC-09`）继续有效 |
...
| 异常 | 适用：阻塞/错误提醒卡不收合（data-wait 作用域外）；非法档位回退完整呈现 | `e2e/specs/error-reminder.mjs#R-01-002/AC-09`、`src/core.mjs::normalizeDensity` |
