...
| R-01-011 | 窗格渲染器 | 标题行整体折叠控件与悬停收起图标 | src/client.mjs |
...
| R-01-022 | 窗格渲染器 | 标题行工具区仓库入口 | src/client.mjs |
## 产品契约
...
- 关键内部结构:
  ...
  - 仓库入口：`.dap-repo` 为标题行右侧工具区（`.dap-tools`）内的常显纯图标链接（与档位切换按钮同视觉语言：22px 圆形、不透明底色、hover/focus-visible 高亮、可访问名称，MIT 许可的 GitHub Octicon `mark-github` 图标），`href` 指向 https://github.com/ccll/dsh-activity-pane 并以 `target="_blank"` + `rel="noreferrer noopener"` 在新标签页打开；链接位于 `.dap-tools` 内、与 `.dap-titlebar` 为兄弟节点，激活不进入标题区收起激活路径，也不改变当前选中会话与呈现档位；桌面折叠窄条态随 `.dap-header` 整体隐藏，移动端抽屉形态同样适用；链接随窗格骨架创建与移除（R-01-022）。
- 代码位置: src/client.mjs
