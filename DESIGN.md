...
- 最近卡弱化且可辨的视觉呈现：整体不透明度 0.8（低于活动卡）承载历史区弱化（R-01-013/AC-10）；卡片底色与细描边保持与窗格底色可分辨——深色主题为 `rgba(31,34,42,0.92)` 底色 + `rgba(255,255,255,0.08)` 描边，浅色主题为 `--dsw-alias-bg-layer-2` 底色 + `--dsw-alias-border-l2` 描边；悬停沿用既有亮度反馈（R-01-013/AC-11）。
