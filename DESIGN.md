...
- 加载状态模型（R-01-014）：
  ...
  - 计数级：列表在途时三处数量标识（列头、窄条、移动端开关）经 `countCapsuleState` 归一为加载指示（spinner），不冒充 0/0 计数；就绪后显示实际 n/m（R-01-014/AC-06）。
