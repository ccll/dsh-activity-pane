...
- 关键内部结构:
  ...
  - 卡片紧凑呈现切换：`.dap-density` 为窗格内 `position:absolute` 的圆形图标按钮，常显于窗格右上角、标题栏正下方（`right:12px`、`top:40px`，悬浮于列表之上），规格与 `.dap-top` 一致（纯图标无文字、`aria-label` 可访问名称、不透明底色、hover/focus 高亮）；激活时按完整→中间→紧凑循环翻转窗格根 `data-density` 属性并执行滚动锚定（补偿 `scrollTop` 使当前选中卡片顶部相对视口位置不变，当前卡不可得时不补偿），可访问名称表达将切换到的目标档位；中间档经 CSS 隐藏时间线与轨迹、进度行、统计行，紧凑档在中间档基础上再隐藏 `.dap-card-head`、等待末行（`.dap-await-head`/`.dap-note-row`）与最近卡消息预览行，仅保留标题行——纯呈现层开关，卡片 DOM 复用、渲染签名与激活跳转逻辑不感知档位（R-01-021/AC-01、AC-02、AC-04、AC-08）；档位经 `normalizeDensity` 归一后存 localStorage，启动恢复，会话状态变化不翻转已选档位；桌面折叠窄条态经 CSS 隐藏，移动端抽屉形态同样适用；按钮随窗格骨架创建与移除（R-01-021、R-02-003）。
- 代码位置: src/client.mjs
