...
状态: active
关联: C-047、C-049、C-051、C-052、C-053 → E2E 验证基建
风险等级: standard
...
- runner 原有最多 3 个全新 Chromium + dsh web 环境；同服务内四次短 reload、多冷环境恢复与双宿主并发均过量。

## 收敛方案

- `openApp` 首代按 DSH unary timeout 加渲染余量等待 32s；rc.7 首败不自重试，因此仅 reload 一次并在已启动服务上等待 12s。
- 两代仍 loading 或明确进入列表加载失败，才抛 `ERR_PANE_STALL`；runner 从最多 3 个环境收紧为 2 个环境。
- 双 worker 下 C-051/C-052 本地仍分别失败 4/10、2/10，按 C-053 固定顺序执行，使本地与 hosted 使用同一单宿主负载模型。
- 增加 helper/runner 源契约，阻止恢复为短世代 reload 循环或第三个冷环境；不修改产品行为或放宽普通 spec 断言。
...
| 维度 | 适用性/理由 | 可执行证据 |
|---|---|---|
| 成功 | 适用：较慢但在 unary timeout 内完成的 sessions.list 不被短周期 reload 中断；首败后新 manager 有一次重拉 | `e2e/helpers.mjs::openApp` + hosted run |
| 异常 | 适用：32s + 一次 12s 重拉后仍 loading 或列表加载失败进入一次 fresh-environment recovery | `e2e/helpers.mjs::ERR_PANE_STALL` |
| 边界配置 | 适用：每个环境最多两个页面连接世代；runner 最多恢复一次 | `scripts/check.mjs::e2eHelperSource`、`e2e/run.mjs::runSpec` |
| 副作用 | 适用：普通断言失败仍不重试，不改变产品 bundle | `e2e/run.mjs::worker` |
| 性能 | 适用：消除 5×6s reload 风暴，首代获得完整 32s，重拉仅 12s；记录本地/hosted 墙钟 | `package.json::scripts`、`.github/workflows/ci.yml::verify` |
| 并发 | 适用：固定顺序执行，使本地/hosted 同时只有一套 dsh web + Chromium | `e2e/run.mjs::MAX_CONCURRENCY` |
| 恢复 | 适用：只保留一次 post-timeout reload 与一次全新 Chromium + dsh web 恢复 | `e2e/helpers.mjs::openApp`、`e2e/run.mjs::runSpec` |
