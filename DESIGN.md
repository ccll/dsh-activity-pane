...
- 关键机制：
  ...
  - 运行卡外观对齐 answer-pet：状态行头文案（使用工具/回答中/思考中，工具名在末尾）、动作时间线（竖线与圆点严格同圆心、竖线终点没入最新动作圆点、圆点半透明外环、正在执行节点闪烁、圆点不被容器裁切）、进度条（5px 圆角、流式阶段内部向右滚动条纹动画，经 `data-streaming` 属性驱动）（R-01-009/AC-02、AC-08、AC-09）。
...
- 流程节点轨迹：`活动状态模型#buildTrace(...)` 产出最近少量流程节点（已定案工具调用 + 当前阶段），含 label/detail/status/durationMs；`summarizeToolArguments` 只从白名单字段提取摘要（R-01-009/AC-07）。渲染层以竖线串圆点的时间线呈现：竖线与圆点严格同圆心、竖线终点没入最新动作圆点内部不外露，圆点带半透明外环、正在执行节点蓝色外环 + 闪烁、已定案节点绿/红实心圆点，且圆点位于内容区不被容器裁切（R-01-009/AC-09）。
